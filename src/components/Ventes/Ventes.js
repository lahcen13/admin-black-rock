...
import FormVenteUpdate from '../Forms/FormVenteUpdate/FormVenteUpdate';
...
const Ventes = (props) => {
  ...
  const [filter,setFilter] = useState({name:"date_livraison"});
  ...
  useEffect(() => {
    var data = {filter:'AND( SEARCH("Validé", {validation_paiement}), SEARCH("Oui", {recuperer_livraison}) '}
    if (filter.value){
      ...
      <div className={styles.Content + " sm:px-6 w-full"}>
        <div className="px-4 md:px-10 py-4 md:py-7">
          <div className="flex items-center justify-between">
            <p tabIndex="0" className="hidden sm:block focus:outline-none text-base sm:text-lg md:text-xl lg:text-2xl font-bold leading-normal text-gray-800" >Les vehicules vendus</p>
            <div className="flex">
              <div className="relative flex items-center text-sm font-medium leading-none text-gray-600 cursor-pointer rounded-xl ">
                <select onChange={(e)=>{setFilter({...filter, name:e.target.value})}} style={{ marginRight: "3px" }} aria-label="select" className="w-full focus-within:text-cyan-400 focus:outline-none bg-transparent border-r border-gray-300 rounded-xl">
                  <option value="date_livraison" className="text-sm text-gray-400">Date de livraison</option>
        ...
        <div className="bg-white py-4 md:py-7 px-4 md:px-8 xl:px-10">
          ...
          <div className="mt-7 overflow-x-auto">
            {!formView ?
              ...
              :
              commandeToUpdate && <FormVenteUpdate suivisToUpdate={commandeToUpdate} setFormView={() => setFormView()} setRefresh={() => setRefresh(!refresh)} />
            }
          </div>
          {!formView ?
            commandes.length > 10 && <Pagination nbrPage={nbrPage} dataLength={dataPagination && dataPagination.length} pagination={pagination} moins={() => { setPagination(pagination - 10); setNbrPage(nbrPage - 1) }} plus={() => { setPagination(pagination + 10); setNbrPage(nbrPage + 1) }} />
            : ""}
        </div>
      </div>
    </>
