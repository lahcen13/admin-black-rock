...
import styles from './VehiculeCommande.module.css';
import axios from 'axios';
import DeleteModal from '../../Elements/DeleteModal/DeleteModal';
import VehiculePrixTotal from '../VehiculePrixTotal/VehiculePrixTotal';
...
const VehiculeCommande = (props) => {
  ...
  useEffect(() => {
    axios.get('https://protected-ravine-10497.herokuapp.com/commandes/' + props.data.commande[0], header).then(
      (response) => {
      ...
      <div className={styles.Content + " sm:px-6 w-full"}>
        <div className="px-4 md:px-10 py-4 md:py-7">
          <div className="flex items-center justify-between">
            <p tabIndex="0" className="focus:outline-none text-base sm:text-lg md:text-xl lg:text-xl font-bold leading-normal text-gray-800"><b>Voiture : </b> {props.data && props.data.marque + " " + props.data.modele + " " + props.data.couleur  }</p>
          
          </div>
          <div>
            <p tabIndex="0" className="focus:outline-none text-base sm:text-lg md:text-l lg:text-xl font-bold leading-normal text-gray-800"> <b> Fintion : </b> {props.data && props.data.finition}<br></br>  <b>Châssis :</b> {props.data && props.data.chassis} </p>
          </div>
        </div>
            ...
            {!formView ?
              <table className="w-full whitespace-nowrap">
                ...
                <tbody>
                      ...
                      {commande.id_client ?
                        <td className="">
                          <div className="text-sm leading-none text-gray-600 ml-2 text-center">
                            <div onClick={() => { props.showClient(); props.idClient(commande.id_client) }}><ClientFullName id_client={commande.id_client} /> </div>
                          </div>
                        </td> :
                  ...
                  )) : !noRecords ? <LoadAnimation/> : <tr><td>Plus de commande</td></tr>}
                </tbody>
              </table>
...
export default VehiculeCommande;
