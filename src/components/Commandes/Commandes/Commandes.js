...
const Commandes = (props) => {
  ...
  const [filter, setFilter] = useState({ name: "id_vehicule" });
      ...
      <div className={styles.Content + " sm:px-6 w-full"}>
        <div className="px-4 md:px-10 py-4 md:py-7">
          <div className="flex items-center justify-between">
            <p tabIndex="0" className="hidden sm:block focus:outline-none text-base sm:text-lg md:text-xl lg:text-2xl font-bold leading-normal text-gray-800" >Les commandes </p>
            <div className="flex">
            ...
            {!formView ?
              <table className="w-full whitespace-nowrap">
                ...
                <tbody>
                  ...
                  {dataPagination  ?  dataPagination.map((commande, key) => (
                    <tr key={key} tabIndex="0" className="focus:outline-none h-16 border border-gray-100 rounded">
                      ...
                      <td className="">
                        {commande.id_vehicule ?
                          <div className="text-sm leading-none text-gray-600 ml-2 text-center">
                            <VehiculePrixTotal id_vehicule={commande.id_vehicule} frais_supplementaire={parseInt(commande.frais_supplementaire)} />
                          </div>
                          :
                          <div className="text-sm leading-none text-gray-600 ml-2 text-center">
                            <p>
                  ...
                  )) : !noRecords ? <LoadAnimation /> : <tr><td>Pas de commande</td></tr>}
                </tbody>
              </table>
              : formView === "add" ? <FormCommandeAdd setFormView={() => setFormView()} setRefresh={() => { setRefresh(!refresh); }} /> : <FormCommandeUpdate commandeToUpdate={commandeToUpdate} setFormView={() => setFormView()} setRefresh={() => setRefresh(!refresh)} />
            }
          </div>
          {!formView ?   
